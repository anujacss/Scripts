...
curl_setopt($ch, CURLOPT_URL, "https://api.litmos.com/v1.svc/users?apikey=3b47e4b8-2da6-4832-b82f-d81abae7110a&source=sampleapp&limit=1000");
...
function getTeamUsers($team){
	...
	curl_setopt($ch, CURLOPT_URL, "https://api.litmos.com/v1.svc/teams/$team/users?apikey=3b47e4b8-2da6-4832-b82f-d81abae7110a&source=sampleapp&limit=1000");
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
...
function getUser($id){
	...
	curl_setopt($ch, CURLOPT_URL, "https://api.litmos.com/v1.svc/users/$id?apikey=3b47e4b8-2da6-4832-b82f-d81abae7110a&source=sampleapp&limit=1000");
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
...
function addUserToTeam($teamId,$userId){
		...
		curl_setopt_array($curl, array(
		  CURLOPT_URL => "https://api.litmos.com/v1.svc/teams/".$teamId."/users?apikey=3b47e4b8-2da6-4832-b82f-d81abae7110a&source=sampleapp",
		  ...
		  CURLOPT_HTTPHEADER => array(
			...
			"Postman-Token: 963341e5-aec3-4a46-bfc4-28718ba4ea67",
...
function UpdateExpirationdate($user,$expirationdate){
	...
	$u_id = $user['Id'];
	curl_setopt_array($curl, array(
	CURLOPT_URL => "https://api.litmos.com/v1.svc/users/$u_id?apikey=3b47e4b8-2da6-4832-b82f-d81abae7110a&source=sampleapp",
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => "",
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 30,
	  ...
	  CURLOPT_HTTPHEADER => array(
		"Content-Type: application/xml",
		"Postman-Token: 809f409d-e4da-40bb-87b1-d28e711e626d",
		"cache-control: no-cache"
	  ),
	));
...
function createUserLitmos($wa_apricot){
		...
		$curl = curl_init();
				curl_setopt_array($curl, array(
				  CURLOPT_URL => "https://api.litmos.com/v1.svc/users?apikey=3b47e4b8-2da6-4832-b82f-d81abae7110a&source=sampleapp",
				  CURLOPT_RETURNTRANSFER => true,
				  CURLOPT_ENCODING => "",
				  CURLOPT_MAXREDIRS => 10,
				  CURLOPT_TIMEOUT => 30,
...
function searchLitmosEmail($email){
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, "https://api.litmos.com/v1.svc/users?apikey=3b47e4b8-2da6-4832-b82f-d81abae7110a&source=sampleapp&search=$email");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
...
<?php
	...
	if(isset($_POST['submit'])){
		...
		if(is_int($user_wa_contacts)){
			...
			echo '<div class="available-wildapricot">User exists in wildapricot</div>';
			...
			if(!$litmoss){
				...
				if(!empty($newuser)){		
							...
							addUserToTeam('G5ZFsJKLTpc1',$newuser->Id);
							addUserToTeam('cNydBDNYDCE1',$newuser->Id);
							$html = 'New user added successfully -  '.$newuser->Email.PHP_EOL;
					        echo '<div class="success_msg">'.$html.'</div>';
					        ?><style>
					        	.available-wildapricot{display:none !important;}
				...
				echo '<div class="mydiv">User already exists in Litmos</div>';
			}

			} else {
				echo '<div class="mydiv">User not exists in wildapricot</div>';
			}

		}
		else{

			echo '<div class="mydiv">Please Enter Email</div>';
