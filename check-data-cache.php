...
/*function createusertribe($wa_apricot){
	//print_r($wa_apricot);
...
$bearer_token = $response['access_token'];	
	...
	$curl = curl_init();
  $content = json_encode(array("username" => ''.$wa_apricot["FirstName"].'_'.$wa_apricot["LastName"].'',"name" => ''.$wa_apricot["FirstName"].' '.$wa_apricot["LastName"].'',"email" => 'ankush.swarnatek@gmail.com',"password" => 'ACss1234$', "role" => 'member', "emailStatus" => 'verified',));
curl_setopt_array($curl, array(
...
$newArr1= json_decode($response, true);
 ...
 $uname = $newArr1['errors']['username'];
 $user_id = $newArr1['id'];
if($uname == 'Username is already in use.'){
	
	echo '<div class="mydiv no"><span>&#10006;</span>User already exists in Tribe</div>';
	
}
elseif($user_id){
	echo '<div class="success_msg yes"><span>&#10004;</span>User Added to Tribe</div>';
}

$org = htmlentities( $wa_apricot['Organization'] );

	$curl = curl_init();
  $content = json_encode(array("title" => ''.$org.''));
curl_setopt_array($curl, array(
...
session_destroy();
	if(isset($_POST['submit'])){
		...
		/*	if(!empty($user_records)){
			$tribe = createusertribe($user_records);
			//print_r($tribe);
			}*/
			if(!$litmoss){
