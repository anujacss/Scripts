 ...
 //print_r($contacts);
	//$wa_contacts = [];
		...
		foreach($contacts as $key => $value){
			...
			$data['hide_community'] = $value['FieldValues'][37]['Value']['Label'];
			$wa_contacts[] = $data;
		}
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://acss.tribe.so/api/v1/oauth/token",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
...
$curl = curl_init();
  $content = json_encode(array("username" => 'swarnatekkffkkkk',"name" => 'swarnatekkkkffkkk',"email" => 'swarnatdffddek@gmail.com',"password" => 'Google@!123',"role" => 'member'));
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://acss.tribe.so/api/v1/users",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
