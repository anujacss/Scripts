...
session_start();
   ...
   echo  $tokennew =  $waApiClient->initTokenByApiKey('9tkmpb73aa5nh19j3qm3hw6y49tps6');
 ...
 print_r($contacts); 
	//$wa_contacts = [];
		...
		foreach($contacts as $key => $value){
			if($value['Email'] == 'itsmeanujn@gmail.com'){
			//echo "<pre>";
 //print_r($value); 
			$data = [];
			...
			$data['profile_img'] = $value['FieldValues'][26]['Value']['Url'];
			$data['profile_img_id'] = $value['FieldValues'][26]['Value']['Id'];
			
			$data['MembershipLevel'] = $value['MembershipLevel']['Id'];	
			$data['city'] = $value['FieldValues'][28]['Value'];
			$data['hide_profile'] = $value['FieldValues'][38]['Value']['Label'];
...
$headers[] = "cache-control: no-cache";
    ...
    class WaApiClient{
       ...
       private function getAuthTokenByAdminCredentials($login, $password)
       {
          ...
          $data = sprintf("grant_type=%s&scope=%s&username=%s&password=%s", 'password', urlencode($login), urlencode($password), urlencode($this->tokenScope));
	 ...
	 function getProfileImage()
    {
       global $waApiClient;
       global $accountUrl;
       $url = $accountUrl . '/pictures/bldficlt.png?fullSize=false&asBase64=false';
       return $waApiClient->makeRequest($url);
    }

	function img($pic_id, $pic_name, $tokennew){
		 ...
		 global $waApiClient;
		  ...
		  $pic_name;
		 if($pic_id){
		 $pic = $pic_id.'?fullSize=true&asBase64=true';
...
<?php
	...
	if(isset($_POST['submit'])){
		...
		$user_records = $wa_contacts[$user_wa_contacts];
		 $pic_id =  $user_records['profile_img'];
$pic_name =  $user_records['profile_img_id'];
 $city =  $user_records['city'];
img($pic_id,$pic_name,$tokennew);
